...
//$url = 'http://api.petfinder.com/shelter.getPets?key=611802de24144ae1b2112c39be644c8b&id=TN198&format=json&count=24';
$url = 'http://api.petfinder.com/' . $action;

$urlParams = array(
  'key' => '611802de24144ae1b2112c39be644c8b', 
  'format' => 'json', 
);

if ( $action == 'shelter.getPets' ) {
    $urlParams['id'] = 'TN198';
    $urlParams['count'] = $count;
} else if ( $action == 'pet.get' ) {
    $catId = $_GET['catId'];
    $urlParams['id'] = $catId;
} else {
    $contents = 'ERROR: Petfinder API action not available';
